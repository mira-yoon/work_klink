...
<body>
	...
	<ul class="search-list-1">
		<li>
			...
			<span>회원구분</span>
			<select name="utCd" class="selectype form_03">
				${utCdSelectBox }
			</select>
		...
		<c:choose>
			<c:when test="${not empty list}"> --%>
				<c:forEach var="dto" items="${list}">
					<tr>
						<td>${dto.rank }</td>
						<td>${dto.nationDTO.natName }[${dto.nationDTO.natNameKr }]</td>
						<td>${dto.userDTO.memId }</td>
						<td>${dto.userDTO.memNameStr }${dto.userDTO.memLastNameStr }</td>
						<td>
							<c:choose>
								<c:when test="${dto.userDTO.authGroupId eq 'UT01' }">
									관리자
								</c:when>
								<c:when test="${dto.userDTO.authGroupId eq 'UT02' }">
									강사
								</c:when>
								<c:when test="${dto.userDTO.authGroupId eq 'UT03' }">
									일반회원
								</c:when>
			...
			<c:otherwise>
				<tr>
					<td align="center" colspan="7"> 정보가 존재하지 않습니다. </td>
				</tr>
			</c:otherwise>
		</c:choose>
		</tbody>
	...
	//<![CDATA[
		...
		function changeFromToDate(type) {
			...
			$("#statisticsFrm").attr("action","/la/statistics/memberLogin");
		...
		function saveExcel(){
			...
			document.statisticsFrm.action = '/la/statistics/memberLoginExcelDown';
		...
		function fn_excel(){
			var reqUrl = "/la/statistics/memberLoginExcelDown";
