...
<script type="text/javascript">
	...
	$(document).ready(function () {
		/* 타이틀 */
		$(document).attr("title","Memberships > Sign Up > Enter your membership information : K-LINK");
	...
	function checkPassword(obj, lang) {
		...
		if (passwd.length < 10 || cnt < 3) { //10자리 고정//2014.05.12 10자리 이상
			if ("KR" === lang) {
				...
				alert(
					"Password must be 10-digit over, and must be composed of at least three kinds of (uppercase / lowercase / numbers / special characters)."
				);
			}
			obj.focus();
			return false;
	...
	function doSave() {
		...
		var memName = $("#firstName").val();
		memName = memName.trim().toLowerCase().replace(/\s/gi, "");
		$("#firstName").val(memName);

		var memLastName = $("#lastName").val();
		memLastName = memLastName.replace(/\s/gi, "").trim().toLowerCase();
		$("#lastName").val(memLastName);
		...
		/*
		if(!emailSubFlag){
			alert("Please enter your SUB EMAIL.");
			return;
		}
		*/
		if (memPassword == '') {
		...
		if (depName == '') {
			alert("Please enter your Department / Division.");
			$("#depName").focus();
			return;
		}
		if (memJob == '') {
		...
		var reqUrl = "/lu/member/insertMember";
	...
	function fn_findOverlap() {
		...
		if (memId == '') {
			...
			alert("You can not enter spaces in the ID.");
			$("#memId", "#frmMember").focus();
			return;
	...
	function checkFile(param) {
		...
		if (fileExt == "exe") {
			alert("File types that can not be registered.");
			$(param).val("");
			return false;
		}
		...
		if (fileSize > 10000000) {
			alert("You can not upload more than 10M of full size.");
			$(param).val("");
			return false;
		} else {
			var filesizeNumber = "";
...
<div id="signUp">
	<span class="guidance">User ID and e-mail cannot be changed.</span>
	<form id="frmMember" name="frmMember" method="post" enctype="multipart/form-data">
		<table>
			<caption class="sr-only">Join membership</caption>
			<tr>
				<th scope="row">User ID <span>*</span></th>
				<td>
					<div class="input-box"><input type="text" title="user id" aria-label="id" id="memId" name="memId" /><input type="hidden" name="resetId" id="resetId" /></div>
					<a href="#" onclick="javascript:fn_findOverlap();" class="btn-blue">Check</a>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="input-box"><input type="Password" title="Password" aria-label="Password" name="memPassword" id="memPassword" /></div>
					<p>Password must be 10 letters over : must include 3 types among upper cases, lower cases, numbers and special
						characters</p>
					<p>※ special characters : ~ ?! @ # $ % ^ & * ( ) _ - + = [ ] [ ] | \ ; :‘ “ <> , . ? / Password combination as
							below can not be used.</p>
					<ol>
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="Password" title="Password Check" aria-label="Password Check" name="memPasswordChk" id="memPasswordChk" />
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="birth">
					...
					<span style="font-size: 10pt; font-weight: bold;">Birth Date must be the form of "yyyy-mm-dd". ex) 1980-01-15</span>
				</td>
			</tr>

			<tr>
				<th scope="row">Passport Copy <span>*</span></th>
				<td>
					<div class="input-file">
						<input type="text" id="pictureName" class="upload-name" disabled="disabled" title="the name of passport copy file" aria-label="the name of passport copy file" />
						<input type="file" accept="image/*" id="picture" name="file_atchFile" class="upload-hidden" onChange="javascript:checkFile(this);" aria-hidden="true" />
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="text" title="Given(First) Name" aria-label="Given(First) Name" name="memNameStr" id="firstName" />
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="text" title="Surname(Last) Name" aria-label="Surname(Last) Name" name="memLastNameStr" id="lastName" />
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="text" title="Organization" aria-label="Organization" name="orgName" id="orgName" />
					</div>
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="text" title="Department / Division" aria-label="Department / Division" name="depName" id="depName" />
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="input-box">
						<input type="text" title="Job title / position" aria-label="Job title / position" name="memJob" id="memJob" />
					</div>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="radio-select">
					<c:forEach var="otlist" items="${otlist}">
						<div class="radio-box">
							<input type="radio" id="${otlist.codeName }" name="orgGubun" title="${otlist.codeName}" aria-label="${otlist.codeName}" value="${otlist.codeCode }" />
			...
			<tr>
				...
				<td class="mail">
					<div class="input-box"><input type="text" title="Email" aria-label="Email" id="email" name="emailStr" /></div>
					<a href="#" onclick="javascript:fn_findOverlapEmail();" class="btn-blue">Check</a>
			...
			<tr>
				...
				<td>
					<div class="input-box"><input type="text" title="Sub Email address" aria-label="Sub Email address" id="emailSub"
							name="emailSubStr" /></div>
			...
			<tr>
				...
				<td>
					<div class="input-box"><input type="text" title="Mobile phone number" aria-label="Mobile phone number" name="hpNoStr" id="hpNo" /></div>
			...
			<tr>
				...
				<td>
					<div class="input-box"><input type="text" title="Business phone number" aria-label="Business phone number" name="telNoStr"
							id="telNo" /></div>
					<p>Country code - City code - Number</p>
				</td>
			</tr>
	...
	<div class="btn-box">
		<a href="#" class="btn-blue" onclick="javascript:doSave();">Join</a>
		<a href="/" class="btn-white">Cancel</a>
	</div>
</div><!-- E : signUp -->
